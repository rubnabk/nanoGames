...
public class NGServerThread extends Thread {
	...
	public void run() {
		try {
			System.out.println("servidor: antes de receive y verify token");
	...
	private void receiveAndVerifyToken() throws IOException {
		...
		while (!tokenVerified) {
			...
			System.out.println("servidor: esperandot token");
			long token = dis.readLong();
			tokenVerified = true;
			System.out.println(token);
			dos.writeBoolean(tokenVerified);
		}
	}
